...
const BookingSummary = ({ tour, guests, date }) => {
  ...
  return (
    <Card className="space-y-6 bg-slate-900 text-slate-100">
      <div>
        <p className="text-xs uppercase tracking-widest text-primary-200">Tóm tắt đặt chỗ</p>
      ...
      <div className="space-y-4 text-sm text-slate-200">
        <div className="flex items-center gap-3 rounded-2xl bg-slate-800 px-4 py-3">
          ...
          <div>
            ...
            <p className="text-slate-400">{guests} người tham gia</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-2xl bg-slate-800 px-4 py-3">
          <CalendarDays className="h-5 w-5 text-primary-300" />
          <div>
            <p className="font-medium">Ngày dự kiến</p>
            <p className="text-slate-400">{date ? formatDate(date) : 'Linh hoạt'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 text-xs text-slate-400">
        ...
        <div className="flex items-start gap-2">
          ...
          <p>Concierge phụ trách điều phối đưa đón, trải nghiệm và các yêu cầu đặc biệt sau khi xác nhận.</p>
        </div>
      </div>
    </Card>
  );
