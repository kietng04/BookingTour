    ...
    <div className="flex flex-col gap-6 p-6 sm:flex-row sm:items-center sm:p-8">
      ...
      <div className="flex-1">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Trusted operator since {operator.founded} · {operator.tourCount}+ bespoke tours crafted
        </p>
        <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-gray-600">
          <span className="inline-flex items-center gap-2">
            <Star className="h-4 w-4 text-amber-400" aria-hidden="true" />
            {operator.rating.toFixed(2)} traveler rating
          </span>
          <span className="inline-flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-brand-500" aria-hidden="true" />
            Response time {operator.responseTime}
          </span>
        </div>
      </div>
